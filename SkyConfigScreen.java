...
public class SkyConfigScreen extends MainConfigScreen {
    ...
    /// Конструктор
    public SkyConfigScreen(Screen parent) {
        super(Text.literal("Better Sky Settings"), parent);
    }

    @Override
    protected void init() {
        if (LiteApiManager.isFeatureBlocked("better_sky")) {
            close();
            return;
        }
    ...
    private void rebuildUI() {
        ...
        timeSlider = createTimeSlider(centerX,currentY);
    ...
    private SliderWidget createTimeSlider(int centerX,int y){
        return new SliderWidget(centerX-40, y, CONTROL_WIDTH, 20, Text.empty(),
            (config.getBetterSkyTime()-1000)/23000.0){

            @Override
            protected void updateMessage(){
                int value=(int)Math.round(1000+this.value*23000);
                value=((value+500)/1000)*1000;
                setMessage(Text.literal(value+" ticks"));
            }
